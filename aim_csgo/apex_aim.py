...
def lock(aims, mouse, top_x, top_y, len_x, len_y, args, pidx, pidy):
    ...
    # print("(len_x * float(x[1]) + top_x - mouse_pos_x) ** 2 + (len_y * float(x[2]) + top_y - mouse_pos_y) ** 2---",(len_x * float(aims_copy[1]) + top_x - mouse_pos_x) ** 2 + (len_y * float(aims_copy[2]) + top_y - mouse_pos_y) ** 2)
    aims_copy = [x for x in aims_copy if
                 x[0] in args.lock_choice and (len_x * float(x[1]) + top_x - mouse_pos_x) ** 2 + (
                         len_y * float(x[2]) + top_y - mouse_pos_y) ** 2 < detect_arange]
    # k = 1 * (1 / args.lock_smooth)
    if len(aims_copy):
        dist_list = []
...
def show_fps(cv2, lock_mode, team_mode, img0, t0):
    ...
    cv2.putText(img0, "FPS:{:.1f}".format(1. / (time.time() - t0)), (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 2,
                (0, 0, 235), 4)
    cv2.putText(img0, "lock:{:.1f}".format(lock_mode), (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 235),
                4)
    cv2.putText(img0, "team:{:.1f}".format(team_mode), (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 235),
                4)
    # 打印fps 控制频率
    if int(count) % 100 == 0:
